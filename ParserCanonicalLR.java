...
public class ParserCanonicalLR {
    ...
    private Sintaxis syntax;
    private ArrayList globalClosure = new ArrayList();
    private Automata LR;
    private HashSet<Produccion> globalActual;
    private ArrayList<TablaParseo> tablaParseo;
    private String nombreArchivo;
    
    public ParserCanonicalLR(ArrayList<Produccion> producciones,Sintaxis syntax,String nombreArchivo) {
    ...
    public void crearTablaParseo(){
            ...
            for (String letra : alfabeto) {
                for (Transicion trans: (ArrayList<Transicion>)estadoActual.getTransiciones()){
                    if (trans.getSimbolo().equals(letra)){
                        ...
                        tablaParseo.add(new TablaParseo(i,letra,determinarOperacion(letra),LR.getEstados().indexOf(trans.getFin())));
            ...
            for (Produccion product : products) {
                ...
                if ((int)product.getItem().getPosicion() == product.getCuerpo().split(" ").length){
                    ...
                    if (indiceBuscado != -1){
                       ...
                       tablaParseo.add(new TablaParseo(i,(String)product.getLookahead(),"r",indiceBuscado));
                ...
                if (indexOfDolar == (int)product.getItem().getPosicion()){
                     tablaParseo.add(new TablaParseo(i,"$","accept",1));
                }
            ...
            if (cantidadReduce>1 && cantidadShift <1)
               error = ("Error reduce/reduce");
                ...
                for (int k = 0; k<tablaParseo.size();k++){
                    TablaParseo it = tablaParseo.get(k);
        ...
        System.out.println("");
        ArrayList<TablaParseo> conv = new ArrayList(ver3);
    ...
    public void generarParser() {
     ...
     public String copiarTablaParseo(){
        ...
        for (int i = 0;i<tablaParseo.size();i++){
            TablaParseo itemActual = tablaParseo.get(i);
   ...
   "\t"+ "public ArrayList<Produccion> searchProductions(String cabeza){"+"\n"+
    ...
    "\t"+"}"+"\n";
     ...
     public void serialize(){
        try
        {
         ...
         new FileOutputStream("ParserGenerado/automataLR.ser");
         ObjectOutputStream out = new ObjectOutputStream(fileOut);
         out.writeObject(LR);
         out.close();
         fileOut.close();
         System.out.println("Serialized data is saved in /ParserGenerado/automataLR.ser");
        }catch(IOException i)
        {
            i.printStackTrace();
        }
    ...
    public TablaParseo buscarItem(String simbolo, int estado){
        for (int i = 0;i<tablaParseo.size();i++){
            if ((int)tablaParseo.get(i).getActualEstado() == estado &&
                tablaParseo.get(i).getSimbolo().equals(simbolo))
                return tablaParseo.get(i);
