 ...
 * @param <T>
 */
public class AFNConstruct<T> implements RegexConstant {
    ...
    public Automata cerraduraKleene(Automata automataFN)
    {
        ...
        for (int i =0; i<anteriorFin.size();i++){
            anteriorFin.get(i).getTransiciones().add(new Transicion(anteriorFin.get(i), anteriorInicio,EPSILON));
            anteriorFin.get(i).getTransiciones().add(new Transicion(anteriorFin.get(i), nuevoFin, EPSILON));
        }
   ...
   public Automata concatenacion(Automata AFN1, Automata AFN2){
       ...
       Automata afn_concat = new Automata();
        ...
        for (i=0; i < AFN2.getEstados().size(); i++) {
            ...
            if (i == AFN2.getEstados().size()-1){
                ...
                {
                    tmp.setTransiciones(new Transicion((Estado) AFN2.getEstadosAceptacion().get(k),AFN1.getEstadoInicial(),EPSILON));
                }
            }
    ...
    public Automata union(Automata AFN1, Automata AFN2){
        ...
        for (int k =0; k<anteriorFin.size();k++)
            anteriorFin.get(k).getTransiciones().add(new Transicion(anteriorFin.get(k), nuevoFin, EPSILON));
         // agregar transiciones desde el anterior AFN 2 al estado final
        for (int k =0; k<anteriorFin.size();k++)
            anteriorFin2.get(k).getTransiciones().add(new Transicion(anteriorFin2.get(k),nuevoFin,EPSILON));
